...
require_once(DIR_WS_LANGUAGES . $result['directory'] . '/modules/payment/machinecoin.php');
...
if ($confirms < MODULE_PAYMENT_MACHINECOIN_API_CONFIRMS) {
    echo "*failed*";
} else {
    $query = xtc_db_query("SELECT orders_id FROM " . TABLE_ORDERS . "
                           WHERE machinecoin_uniqid = '" . $uniqid . "'
                             AND machinecoin_amount = '" . $amount . "'
                           LIMIT 1");
    ...
    if(!$order) {
        ...
        xtc_db_query("UPDATE " . TABLE_ORDERS . "
                      SET orders_status = '" . MODULE_PAYMENT_MACHINECOIN_PAID_STATUS . "'
                      WHERE orders_id = '" . $order['orders_id'] . "'");

        xtc_db_query("INSERT INTO " . TABLE_ORDERS_STATUS_HISTORY . "
                      (orders_id, orders_status_id, date_added, customer_notified, comments)
                      VALUES
                      ('" . $order['orders_id'] . "', '" . MODULE_PAYMENT_MACHINECOIN_PAID_STATUS . "',
                      now(), 1, '" . MODULE_PAYMENT_MACHINECOIN_PAID_COMMENT . "')");
